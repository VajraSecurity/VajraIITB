...
regions_path = "./collector/regions.npy"
aws_test_path = "./collector/aws_test.npy"
gfs_test_path = "./collector/gfs_test.npy"
encoding_test_path = "./collector/encoding_test.npy"

regions = np.load(regions_path, allow_pickle=True)
aws = np.load(aws_test_path, allow_pickle=True)
gfs = np.load(gfs_test_path)
encoding = np.load(encoding_test_path)
...
while True:
    try:
        ...
        for i in range(aws.shape[0]):
            aws_i = np.expand_dims(aws[i], axis=0)
            gfs_i = np.expand_dims(gfs[i], axis=0)
            encoding_i = np.expand_dims(encoding[i], axis=0)
...
# 
            ...
            np.save(aws_regions_save_path, regions)
            np.save(aws_array1_save_path, aws_i)
            np.save(gfs_array_save_path, gfs_i)
            np.save(encoding_features_save_path, encoding_i)
